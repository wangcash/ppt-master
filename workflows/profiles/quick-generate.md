...
| Delivery | Hand-author the roster, run the §3 early gate on rosters of seven or more pages plus one lockless final checker, stop at the SVG 预览确认门, skip `finalize_svg.py`, and only after user confirmation export with `--quick-generate` |
...
⛔ **BLOCKING — SVG 预览确认门（Quick 第一段结束）**：最终 checker 通过后立即停止，不执行 `svg_to_pptx.py`。向用户报告：SVG 预览已就绪（项目路径 + 页数）、宿主内置预览面板已打开（本环境禁用 `127.0.0.1:6060` live preview，不要启动或报告 live preview URL），并说明用户检查预览、提出修改意见后，点击「生成 PPT 文件」或回复“生成 PPT 文件”再继续导出。收到确认前，Notes / Motion / `svg_to_pptx.py` 均不得执行。

用户在此阶段提出“修改第 X 页 / 只改第 X 页 / 给第 X 页加图”等单页修改意见时，转入 [`edit-single-page`](../stages/edit-single-page.md) 执行；修改完成后仍回到本预览确认门。

**Notes** (when enabled): 仅当用户已确认导出后，load [`executor-notes.md`](../../references/executor-notes.md) after the passing check, validate a frozen script or pre-SVG narration without regenerating it or otherwise generate `notes/total.md` from the final roster, then split:
...
```markdown
## ✅ Quick Step 3 Complete（SVG 预览已就绪）
...
- [ ] **Next**: 等待用户检查预览并提出修改意见；用户点击「生成 PPT 文件」后继续 §4

## ✅ Quick Generate Complete（用户确认后）
