...
**Design Parameter Confirmation (Mandatory)**: before the first SVG, output one confirmation listing the compact communication objective, canvas dimensions, body font size, color scheme (primary/secondary/accent HEX), font plan, and the per-role calibration table. The calibration table comes from `python3 ${SKILL_DIR}/scripts/text_measure.py calibrate <project_path> --outline`: every lock role with family, size, CJK and Latin ≈ chars per 100 px, and the longest planned §IX line per role in px — the checker's own estimator with wrapping headroom, written to `validation/text_calibration.json`. Do not report any `127.0.0.1:6060` / live-preview URL in this environment.

**Live Preview Auto-Startup (DISABLED in this environment)**: do **not** start `svg_editor/server.py` or any browser preview server on port `6060`. The host app already renders the generated SVGs through its built-in PPT preview panel (`/api/v2/ppt-preview`), so no local Flask preview server is needed. Before the first SVG, if an old preview server is still running for this project, stop it once with:

```bash
python3 ${SKILL_DIR}/scripts/svg_editor/server.py <project_path> --shutdown
```

Do not report or infer a `6060` URL. Do not open a browser. Do not read or apply submitted annotations during generation; that window opens after Step 7 ([`live-preview.md`](stages/live-preview.md)).
...
**✅ Internal checkpoint** — preview launched in time, P01 method gate, uninterrupted remaining pages, consolidated repair, exact §IX coverage, one-frame prose, final checker 0 errors, `notes/total.md` only when enabled. Do not print.

> **Chart pages?** Run [`verify-charts`](stages/verify-charts.md) before the preview gate to calibrate coordinates; skip without chart pages.
>
> **Visual self-check (opt-in)?** Run [`visual-review`](stages/visual-review.md) before the preview gate only when the user explicitly asked for a per-page visual re-pass ("跑一下视觉自检 / 视觉回看", "visual review", "check pages visually"); never by default, on inferred model capability, or on deck size.
>
...
⛔ **BLOCKING — SVG 预览确认门（两段式交付第一段结束）**：Step 6 最终质检通过后立即停止，不执行 Step 7.3 的 `svg_to_pptx.py`。向用户报告：
1. SVG 预览已就绪（项目路径 + 页数）；
2. `live_preview` URL（如已启动）或预览面板已打开；
3. 请用户检查预览并提出修改意见；确认无误后点击「生成 PPT 文件」或回复“生成 PPT 文件”，再执行 Step 7 导出。

收到用户确认前，禁止进入 Step 7（`total_md_split.py` / `finalize_svg.py` 可在确认前执行以完善预览，但 `svg_to_pptx.py` 必须等确认）。

用户在此阶段提出“修改第 X 页 / 只改第 X 页 / 给第 X 页加图”等单页修改意见时，转入 [`edit-single-page`](stages/edit-single-page.md) 执行；修改完成后仍回到本预览确认门。

---

### Step 7: Post-processing & Export

🚧 **GATE**: 用户已点击「生成 PPT 文件」/ 明确回复“生成 PPT 文件”或“导出 PPTX”。未收到该确认时不得进入 Step 7。
...
## ✅ Step 6 Complete（SVG 预览已就绪）

- [x] Image readiness gate passed（需要手动素材的，预览确认门之前必须补齐）
...
- [x] 已停止于 SVG 预览确认门，未执行 `svg_to_pptx.py`
- [ ] **Next**: 等待用户检查预览并提出修改意见；用户点击「生成 PPT 文件」后进入 Step 7

## ✅ Generate PPTX Complete（用户确认后）
