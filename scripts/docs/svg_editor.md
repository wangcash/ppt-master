...
> **deeppath-agent environment**: the browser SVG editor on `127.0.0.1:6060` is **disabled** in this environment. The workflows do not launch it; use the host app's built-in PPT preview panel for SVG preview. `--shutdown` remains available to stop a legacy server.
